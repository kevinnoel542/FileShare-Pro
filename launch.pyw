...
def update_app():
    ...
    # Example: pull latest code (you can replace this with real logic)
    # subprocess.run(["git", "pull"])
    # or download/update something
...
def run_tray():
    ...
    def quit_app(icon, item):
        log("Quitting app and shutting down server...")
        kill_flask()
        time.sleep(0.5)
        icon.stop()
        log("Shutdown complete.")
        os._exit(0)  # Force quit everything
    ...
    def update_app_menu(icon, item):
        log("Triggered update from menu.")
        kill_flask()
        update_app()
        start_server()
    ...
    icon.menu = pystray.Menu(
        ...
        item("Update App", update_app_menu),
